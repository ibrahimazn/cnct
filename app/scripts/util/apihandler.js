(function(angular) {
    'use strict';
    angular.module('slmlp').service('apiHandler', ['$http', '$q', '$window', '$translate', '$httpParamSerializer', 'slmlpService', 'Upload',
        function ($http, $q, $window, $translate, $httpParamSerializer, slmlpService, Upload) {
        ...
        ApiHandler.prototype.list = function(apiUrl, path, customDeferredHandler, exts) {
            var hasList = slmlpService.crudService.listAllByGateway("containers/container/api/files/lists", "&path="+path);
            var results = [];
