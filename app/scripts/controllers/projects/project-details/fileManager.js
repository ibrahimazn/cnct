function fileManagerDetailsCtrl($scope, toastr, $stateParams, slmlpService, $http, $timeout, $rootScope, $window, $translate, fileManagerConfig, item, fileNavigator, apiMiddleware, $location, $state, localStorageService) {
    /** Generic object for the controller. */
    var $storage = $window.localStorage;
    $scope.config = fileManagerConfig;
    $scope.config.basePath = slmlpService.appConfig.CURRENT_PROJECT;
    ...
    $scope.cloneRepo = function () {
        var hasList = slmlpService.crudService.listAllByGateway("containers/container/api/files/actions", "&path=/");
        var results = [];
    ...
    $scope.getUnCommittedChanges = function () {
        var hasList = slmlpService.crudService.PostViaGateway("containers/container/api/files/commitfiles", $scope.gitObject);
        var results = [];
        hasList.then(function (result) {
            $scope.unCommitedChanges = result;
        });
    ...
    $scope.gitPull = function (size) {
        var hasList = slmlpService.crudService.PostViaGateway("containers/container/api/files/pull", $scope.gitObject);
        hasList.then(function (result) {
    ...
    $scope.uploadFile = function (event) {
        ...
        if (event.target.files.length == 0) {
            ...
            var hasUploaded = slmlpService.promiseAjax.uploadFile(event.target.files[0], $scope.gitObject.podUuid, $scope.gitObject.pvc, $scope.gitObject.node, slmlpService.appConfig.CURRENT_PATH, slmlpService.localStorageService, "containers/container/api/files/uploadFile")
    ...
    $scope.exploreCsvToTable = function () {
        ...
        var hasResult = slmlpService.crudService.listAllByGateway("containers/container/api/files/getcsvfile", additionalParams);
...
//            $scope.filerManagerDetailsObject.getLaunchersList();
    ...
    $scope.filerManagerDetailsObject = {
        ...
        launchFile: function () {
            var hasResult = slmlpService.crudService.listAll("launcher/file/launch");
            hasResult.then(function (result) {
            });
        },
        getFile: function () {
            var hasResult = slmlpService.crudService.listAll("launcher/file-manager/launch?type=File");
            hasResult.then(function (result) {
            });
        },
...
//                }
        ...
        webSocketEvents: function () {
            $scope.$on("Deployment_ScalingReplicaSet", function (event, args) {
                ...
                $scope.launcherUrl = 'http://' + slmlpService.appConfig.K8S_IP + ':' + args.metadata.selfLink;
        ...
        startLauncher: function (size) {
            slmlpService.dialogService.openDialog("views/projects/project-details/file-manager/start-launcher.html", size, $scope,
                    ['$scope', '$uibModalInstance', '$rootScope', function ($scope, $uibModalInstance, $rootScope) {
                            $scope.cancel = function () {
                                $uibModalInstance.dismiss('cancel');
                            };
        ...
        init: function () {
            ...
            if ($scope.projectId > 0) {
                slmlpService.localStorageService.set("projectId", $scope.projectId);
                var hasServer = slmlpService.crudService.read("project", $scope.projectId);
            ...
            $scope.filerManagerElement.launchersLists = $scope.project.launchers;
//            $scope.filerManagerDetailsObject.getLaunchersList();
