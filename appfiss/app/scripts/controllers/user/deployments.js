function deploymentsDetailsCtrl($scope, $state, appfissService, $http, toastr) {
    ...
    $scope.deploymentsObject = {
        ...
        openChooseFileWindow: function(size, type) {
            appfissService.dialogService.openDialog("views/projects/project-details/model/train/choose-dataset.html", size, $scope,
                ['$scope', '$uibModalInstance', '$rootScope', function ($scope, $uibModalInstance, $rootScope) {

                $scope.chooseFile = function() {

                    $scope.deployment[type] = angular.copy(appfissService.appConfig.CURRENT_FILE);
        ...
        createDeployment: function (size) {
            appfissService.dialogService.openDialog("views/projects/project-details/deployments/create.html", size, $scope,
                    ['$scope', '$uibModalInstance', '$rootScope', function ($scope, $uibModalInstance, $rootScope) {
        ...
        scoreDataset: function (size) {
            appfissService.dialogService.openDialog("views/projects/project-details/scoring/score.html", size, $scope,
                    ['$scope', '$uibModalInstance', '$rootScope', function ($scope, $uibModalInstance, $rootScope) {
                            $scope.deploymentsObject.save = function (analyze) {
                                $scope.createModelForm.submitted = true;
                                if (true) {
            ...
            appfissService.dialogService.openDialog("views/delete-confirm.html", size, $scope,
                    ['$scope', '$uibModalInstance', '$rootScope', function ($scope, $uibModalInstance, $rootScope) {
                            ...
                            $scope.confirmDelete = function () {
                                ...
                                var hasResult = appfissService.crudService.PostViaGateway("containers/container/api/modelDeployment/delete", modelDeployment);
        ...
        modelList: function() {
            var hasList = appfissService.crudService.listAllByGateway("containers/container/api/model/findbyproject", "&projectId="+$scope.projectId);
            var results = [];
        ...
        imageList: function() {
            var hasList = appfissService.crudService.listAllByGateway("containers/container/api/launchers/listbytype",
            "&type="+appfissService.appConfig.IMAGE_TYPE.MODEL_PREDICTION);
            var results = [];
            hasList.then(function (result) {
        ...
        getVersionList: function (model) {
            var hasList = appfissService.crudService.listAllByGateway("containers/container/api/modelversions/listbymodel", "&modelId="+model.id);
            var results = [];
            hasList.then(function (result) {
        ...
        create: function() {
            ...
            if($scope.deploymentForm.$valid) {
                ...
                var hasResult = appfissService.crudService.PostViaGateway("containers/container/api/modelDeployment/saveOffline", modelDeployment);
                hasResult.then(function (result) {
...
//                    $state.reload();
        ...
        deploymentList: function() {
            ...
            var hasList = appfissService.crudService.listAllByGateway("containers/container/api/modelDeployment/listByProject", "&projectId="+ $scope.projectId)
            var results = [];
            hasList.then(function (result) {
