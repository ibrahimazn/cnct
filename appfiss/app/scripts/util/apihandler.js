(function(angular) {
    'use strict';
    angular.module('appfiss').service('apiHandler', ['$http', '$q', '$window', '$translate', '$httpParamSerializer', 'appfissService', 'Upload',
        function ($http, $q, $window, $translate, $httpParamSerializer, appfissService, Upload) {
        ...
        ApiHandler.prototype.list = function(apiUrl, path, customDeferredHandler, exts) {
            var hasList = appfissService.crudService.get("views/json/file-manager.json");
            var results = [];
            var list = $http.get("views/json/file-manager.json");
            return list;
        };
