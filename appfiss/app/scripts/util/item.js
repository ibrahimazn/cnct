(function(angular) {
    'use strict';
    angular.module('appfiss').factory('item', ['fileManagerConfig', 'chmod', function(fileManagerConfig, Chmod) {

        var Item = function(model, path) {
            var rawModel = {
                name: model && model.name || '',
                path: path || [],
                type: model && model.isDir || false,
                size: model && parseInt(model.size || 0),
                date: model && model.lastModifiedDate,
                perms: new Chmod(model && model.permission),
